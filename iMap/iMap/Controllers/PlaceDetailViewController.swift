...
final class PlaceDetailViewController: UIViewController {
    ...
    @objc func directionButtonTapped(_ sender: UIButton) {
        ...
        guard let url = URL(string: "maps:/?daddr=\(coordinate.latitude),\(coordinate.longitude)") else { return }
        UIApplication.shared.open(url, options: [:], completionHandler: nil)
    }
    
    @objc func callButtonTapped(_ sender: UIButton) {
        _ = place.location.coordinate
    ...
    private func setupUI() {
        ...
        callButton.addTarget(self, action: #selector(callButtonTapped), for:
                .touchUpInside)
